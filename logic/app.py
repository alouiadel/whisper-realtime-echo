...
def WhisperApp(page: ft.Page):
    ...
    file_section, selected_file_path, selected_file_name, record_button, stop_button, select_file_button = (
        create_file_section(file_picker, lambda: update_ui_state(), start_recording, stop_recording)
    )
    ...
    controls_section, transcribe_button, progress_ring, status_text, vad_checkbox, translate_checkbox, live_button = (
        create_controls_section()
    )
    ...
    def toggle_live_transcription(_):
        ...
        if is_live_active:
            ...
            language = (
                None if model_selector.language_dropdown.value == "auto" else model_selector.language_dropdown.value
            )
    ...
    history_dialog, open_history_dialog = create_history_dialog(page, clipboard_history, on_history_item_copy)
    ...
    whisper_service = WhisperService(
        on_status_update=on_status_update, on_result=on_result, on_error=on_error, on_complete=on_complete
    )
