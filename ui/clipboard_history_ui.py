...
def create_history_dialog(page: ft.Page, clipboard_history: ClipboardHistory, on_copy: callable):
                    ...
                    [
                        ft.Row(
                            [
                                ...
                                ft.Text(
                                    item.get("model_name", ""), size=12, color=AppThemeLang.SECONDARY_COLOR, italic=True
                                ),
                            ],
    ...
    def refresh_history():
        ...
        if not history_items:
            list_container.controls = [ft.Text("No history items found", italic=True, color=ft.colors.GREY_500)]
        else:
            list_container.controls = [create_history_item(item) for item in history_items]
